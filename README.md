...
|---|---|
| **[Hybrid Confidential Azure DevOps — self-hosted ADO Server on a Confidential VM with confidential ACI build agents](/usecase-ado-server-iaas/README.md)** 🆕 | A **hybrid ADO solution** that brings confidential computing to CI/CD for services like ADO (SaaS) that don't (yet) directly support ACC: you self-host **Azure DevOps Server** on an AMD SEV-SNP **Confidential VM** (no public IP, reachable only over a private VNet via Azure Bastion) and pair it with **confidential ACI build agents** that self-register over that private network — the control plane *and* the build agents both run inside attested TEE hardware, so your source, pipelines, and build workloads stay protected in use end-to-end. Ships with an end-to-end deploy script set (CVM + VNet + Bastion + NAT egress + agent images) and a sample pipeline. Includes the **[SecretApp](/usecase-ado-server-iaas/pipelines/secretapp-helloworld/README.md)** demo, which shows the confidential pipeline enforcing **attestation-gated Secure Key Release** end to end: a secret sealed into the image is only unsealed when the *exact* attested image + CCE policy releases the Key Vault wrap key (release policy bound to `x-ms-sevsnpvm-hostdata` = sha256(CCE policy)), so a privileged operator who swaps the image or edits the policy gets nothing — see the [trust model & malicious-admin table](/usecase-ado-server-iaas/pipelines/secretapp-helloworld/README.md#trust-model--secure-key-release). |
