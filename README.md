...
| **[CVM samples now support NVIDIA H100 confidential GPU VMs](/vm-samples/README.md#gpu-mode--h100-confidential-gpu-vm)** 🆕 | [`BuildRandomCVM.ps1`](/vm-samples/BuildRandomCVM.ps1) `-GPU` switch builds an `Standard_NCC40ads_H100_v5` confidential GPU VM (AMD SEV-SNP CPU TEE + H100 in CC mode), installs the NVIDIA open-kernel driver and the [`NVIDIA/nvtrust`](https://github.com/NVIDIA/nvtrust) local GPU verifier inside the VM, and runs **two** attestations end-to-end: a GPU CC-mode attestation (`verifier.cc_admin`) and the normal SEV-SNP CPU attestation. Pre-flight check rejects regions/subscriptions that don't have `StandardNCCads2023Family` quota. |
...
- **BuildRandomCVM.ps1** 🆕 *Updated June 2026* — Deploy CVMs with **Confidential OS disk encryption bound to a Customer Managed Key (CMK)** and automated in-VM attestation. The OS disk and VM Guest State (vTPM + Secure Boot) are encrypted with an HSM-backed RSA-3072 key in your Key Vault Premium; the key is only released after Microsoft Azure Attestation (MAA) verifies the VM is a genuine SEV-SNP / TDX CVM, so even the Azure host fabric cannot read the disk. See [What is Confidential OS disk encryption with CMK?](/vm-samples/README.md#what-is-confidential-os-disk-encryption-with-cmk) and [`https://aka.ms/accdocs`](https://aka.ms/accdocs).
  ...
  - **NVIDIA H100 confidential GPU VMs** via the new `-GPU` switch (`Standard_NCC40ads_H100_v5`, family `StandardNCCads2023Family`) — overrides VM SKU + image, installs the open-kernel NVIDIA driver and the [`NVIDIA/nvtrust`](https://github.com/NVIDIA/nvtrust) local GPU verifier inside the VM, and runs both a GPU CC-mode attestation (`verifier.cc_admin`) **and** a SEV-SNP CPU attestation. See [GPU mode — H100 confidential GPU VM](/vm-samples/README.md#gpu-mode--h100-confidential-gpu-vm).
