...
## Restore / Re-seed Database Entries

A fresh `-Deploy` seeds the 100 records from `seed-data.sql` automatically. If you ever
need to restore the seed data into an **already-running** instance (for example, after the
records were deleted, or an older deployment came up empty), you cannot reach SQL Server
directly: the SQL passwords are ephemeral (regenerated each deploy and never saved) and the
Confidential ACI container blocks `az container exec` via its security policy.

Instead, re-seed through the application's own write endpoint using the helper script
[`../seed-live.ps1`](../seed-live.ps1), which parses `seed-data.sql` and `POST`s each record
to `/citizen/new`:

```powershell
# from the sov-examples directory
$cfg = Get-Content .\citizen-registry-config.json -Raw | ConvertFrom-Json
.\seed-live.ps1 -BaseUrl ("http://" + $cfg.fqdn)
```

The script reports how many records were seeded (e.g. `Seeded: 100  Failed: 0`). It is safe
to point at any reachable instance by passing its URL directly:

```powershell
.\seed-live.ps1 -BaseUrl "http://<app-fqdn-or-ip>"
```

> Note: inserts are additive. Records with a `national_id` that already exists will fail the
> `UNIQUE` constraint (counted under `Failed`), so the registry will not accumulate duplicates.
...
| `../seed-live.ps1` | Re-seeds a running instance from `seed-data.sql` via the `/citizen/new` API (see [Restore / Re-seed Database Entries](#restore--re-seed-database-entries)) |
