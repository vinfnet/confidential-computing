#!/usr/bin/env python3
"""
SecretApp entrypoint — PID 1 inside the confidential container.

The CCE policy produced by `az confcom acipolicygen` denies exec_in_container,
so this is the ONLY process tree the container ever runs. Before handing off to
the Flask app it performs Secure Key Release (SKR):

  1. Fetch a fresh MAA SEV-SNP attestation token using the get-snp-report binary
     baked into the image plus the THIM cert chain + UVM endorsements the ACI
     control plane drops into /security-context-*.

  2. Call Azure Key Vault /release for the RSA wrapping key. The key's release
     policy binds:
       - x-ms-attestation-type        == "sevsnpvm"
       - x-ms-compliance-status       == "azure-compliant-uvm"
       - x-ms-sevsnpvm-is-debuggable  == false
       - x-ms-sevsnpvm-hostdata       == <SHA-256 of the CCE policy>
     so the key only releases inside THIS exact container, on AMD SEV-SNP, when
     the CCE policy hash matches.

  3. Use the released RSA key to unwrap the AES-256-GCM data-encryption key, then
     decrypt /app/sealed-data.enc into /run/sealed (an in-memory tmpfs). The
     welcome secret is only ever materialised inside the TEE.

On any failure the container exits non-zero and the ACI runtime restarts it.
There is NO fallback path that runs the app without the released secret — that
is the whole point of attestation-gating.
"""
...
# Ephemeral RSA wrap key. AKV /release wraps the HSM-released key with this key
# using CKM_RSA_AES_KEY_WRAP so it never leaves the TEE in the clear.
...
def get_akv_token() -> str:
    ...
    has 'get' + 'release' on the SKR key. IMDS is reachable from inside the TEE."""
    r = requests.get(
...
def unseal_bundle(rsa_priv) -> dict:
    """Sealed bundle format (versioned, length-prefixed):
      ...
      [..]          ct_len      uint32 LE
      [..]          ciphertext  AES-256-GCM(plaintext, aad=b"secretapp/v1")
    """
    ...
    plaintext = AESGCM(dek).decrypt(nonce, ct, b"secretapp/v1")
...
def write_unsealed(unsealed: dict, key_version: str, release_policy_sha256: str) -> None:
    ...
    for name, content_b64 in files.items():
        # Reject path traversal — names come from the sealed bundle but we treat
        # them as untrusted just in case.
...
# ---------------------------------------------------------------------------
def main() -> int:
    log(f"SecretApp starting; MAA={MAA_ENDPOINT} AKV={AKV_ENDPOINT} key={KEY_NAME}")
    try:
        ...
        log(f"FATAL: {exc}")
        # Exit non-zero so the runtime restarts. NEVER fall through to the app
        # without the released secret.
        return 2
